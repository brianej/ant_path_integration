...
PN = [ones(numPN,1), zeros(numPN,1)]*(-60);
KC = [ones(numKC,1), zeros(numKC,1)]*(-85);
EN = ([1, 0])*(-60);
...
t_spike_KC = ones(numKC,1)*(-10000);
t_spike_EN = ones(numEN,1)*(-10000);
% general spike Recordings:
spike_time_PN = zeros(numPN, interval/dt);
spike_time_KC = zeros(numKC, interval/dt);
spike_time_EN = zeros(numEN, interval/dt);
pre_post_spike_occured = zeros(numKC, numEN);
% time difference
delta_t = zeros(numKC, numEN);
PN_spikes = zeros(numPN, numKC);
...
for idt = 1 : interval/dt
    ...
    synapses_PN_KC = arrayfun(@PN_KC_synapse, dt * ones(size(PN_spikes)), PN_spikes, synapses_PN_KC);
    % Recordings:
    ...
    [synapses_KC_EN, weight_matrix_KC_EN, synaptic_tag_KC_EN, concentration_BA_KC_EN] = arrayfun(@KC_EN_synapse, dt * ones(size(spike_KC)), spike_KC, synapses_KC_EN, weight_matrix_KC_EN, synaptic_tag_KC_EN, delta_t, pre_post_spike_occured, concentration_BA_KC_EN, BA * ones(size(spike_KC)));
    I_KC = sum(g_PN_KC*bsxfun(@times, synapses_PN_KC, (0-KC(:,1))'))';
    I_EN = weight_matrix_KC_EN'*synapses_KC_EN*(0-EN(1));
    
    % update PN neurons
    [spike_PN, PN(:,1), PN(:,2)] = arrayfun(@PN_neuron, dt * ones(size(PN, 1), 1), PN(:,1), PN(:,2), I_PN);
    spike_time_PN(:,idt) = spike_PN;

    % update KC neurons
    [spike_KC, t_spike_KC, KC(:,1), KC(:,2)] = arrayfun(@KC_neuron, dt * ones(size(KC, 1), 1), t * ones(size(KC, 1), 1), KC(:,1), KC(:,2), I_KC, t_spike_KC);
